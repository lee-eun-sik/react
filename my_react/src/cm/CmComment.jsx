...
import {useCommentCreateMutation, useCommentDeleteMutation, useCommentUpdateMutation} from "../features/board/boardApi";
...
const CmComment = ({comments, user, boardId, refetchComments}) => {
    ...
    const handleCommentSubmit = async () => {
        ...
        try {
            const commentDate = {boardId, content : commentText};
            if (parentCommentId) {
                commentDate.parentCommentId = parentCommentId; // 부모 댓글 ID가 있으면 포함
            } else {
                commentDate.parentCommentId = 0; // 
            }
            await createComment(commentDate);
        ...
        showConfirm("정말 삭제하시겠습니까?", async () => {
            {
                try {
                    ...
                    if (refetchComments) refetchComments(); // 댓글 목로 새로 고침
                } catch (error) {
                    showAlert("댓글 삭제 실패");
                }
            }
    ...
    const handleCommentUpdate = async () => {
        ...
        try {
            await updateComment({commentId: editingCommentId, content: editCommentText});
            setEditingCommentId(null); // 수정 모드 종료
            setEditCommentText(""); // 수정된 내용 초기화
            if (refetchComments) refetchComments(); // 댓글 목록 새로 고침
        } catch (error) {
    ...
    const handleReplyComment = (commentId) => {
        setParentCommentId(commentId); // 부모 댓글 ID를 성정하여 자식 댓글 작성 모드로 전환
    };

    // 댓글을 재귀적으로 랜더링하는 함수
    const renderComments = (commentslist, parentId = 0) => {
        return commentslist
            .filter(comment => comment.parentCommentId === parentId) // 부모 댓들 ID 기준으로 필터링
            .map((comment) => (
                <Box 
                    ...
                    <Box>
                        ...
                        {parentCommentId === comment.commentId && user && (
                            <Box mt={2} display='flex' gap={1}>
                                ...
                                <Button size="small" onClick={() => setParentCommentId(null)} color="error">
                                    취소
                                </Button>
                            </Box>
                        )}
                        ...
                        {/* 자식 댓글 표시 (재귀 호출) */}
                        {renderComments(commentslist, comment.commentId, depth + 1)} {/* 부모 ID를 기준으로 자식 댓글 렌더링 */}
                        </Box>
                    ));
                };
